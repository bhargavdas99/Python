...
# ----------------------------------TIME,TEMPERATURE,ALARM----------------------------------------------
            ...
            if r.status_code != 200:
                ...
                speak("Sorry, I couldn't retrieve the temperature information.")
            
            data = BeautifulSoup(r.text, "html.parser")

            try:
                # Adjusting the span search to be more specific
                temperature_span = data.find("span", {"class": "wob_t", "id": "wob_tm"})
                if temperature_span:
                    temperature = temperature_span.text
                    print(f"The temperature is {temperature}°C at your location.")
                    speak(f"The temperature is {temperature}°C at your location.")
                else:
                    raise ValueError("Temperature span not found")
            except Exception as e:
